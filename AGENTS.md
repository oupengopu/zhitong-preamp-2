...
- 频谱显示层必须有独立快起慢落平滑和峰值衰减; 不要把每帧整数值直接硬写到 LVGL bar, 否则实机跳动会不丝滑。
- 16kHz/6.25kHz 的显示底噪必须比低频扣得更多、倍率更低; 实机若高频柱长期偏高, 优先调显示侧 `visual_floor/visual_gain_q8`, 不要动 MSGEQ7 驱动 raw 缩放。
- 蓝表头 VU 指针不要只用七段平均值, 应使用平均值 + 峰值并保持快起慢落, 否则实机会像表针卡在一个位置。
- 蓝表头指针必须使用 220°~320° 的表盘弧线角度并显式设置位置、pivot、颜色和 3px 高度; 不要回到 -42°~+42° 水平线摆法, 高电平会往表盘下方转且容易看不见。
...
- 更新 LVGL 频谱条 (L/R 独立, 含 peak 保持线; 显示层按音量联动放大, 快起慢落平滑, 不影响信号判定)
