...
1. **BLE API ??**: `esp32_ble/ble.h` ? `add_gap_event_callback`/`add_gap_scan_event_callback`/`add_gattc_event_callback`
   ???????? (`ESPHOME_ESP32_BLE_GAP_EVENT_HANDLER_COUNT` ?) ??????????????????
   **??**: `PGA/ble_hid_host.h` ??? `ble->add_gap_event_callback(...)` ????????? ESP-IDF
   `esp_ble_gap_register_callback` / `esp_ble_gattc_register_callback`?

2. **UnicodeDecodeError**: PlatformIO ????? GBK ?????`esphome/util.py:180` ? `s.decode()`
   ?? UTF-8 ?????**??**: `s.decode(errors="replace")`?

3. **HID_EVT ????**: `ble_hid_host.h` ?? `HID_EVT_PLAY_PAUSE`/`SWIPE_*`/`CAMERA*`/`OK` ????
   YAML ???????????**??**: ?????

4. **settings_ui::init ?????**: `settings_ui.h` ?? 20 ??(15?+5bar)?YAML ??? 18 ??
   (? spectrum_auto/spectrum_theme)??? bar ???????**??**: YAML ???? nullptr ???
