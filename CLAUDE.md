...
- LVGL 绘制前使用显示专用视觉均衡: 低/中频提高可见度但用软膝压缩避免满格, 6.25k/16k 扣显示底噪并降低倍率; 频谱柱按当前音量做显示缩放, 信号检测仍使用未放大的 raw frame 平均值
- 更新 LVGL 频谱条 (L/R 独立, 含显示层时间常数平滑和峰值保持线; 火花样式不再叠加人工抖动)
...
- 弱信号判定使用 `s_signal_avg >= 2` + 1.5 秒保持窗口, 避免音乐间隙或弱信号抖动反复清零计时
- 静音/待机/系统未就绪时不自动跳转
- 设置页/大字音量页不受影响
