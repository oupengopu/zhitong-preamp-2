...
- 弱信号判定使用 `s_signal_avg >= 2` + 5 秒保持窗口, 且 `s_signal_avg` 同时参考七段平均值和单频段峰值; 当前输入 MCP 检测无信号时强制不触发自动跳转
- 静音/待机/系统未就绪时不自动跳转
- 设置页/大字音量页不受影响
...
> 恢复出厂会重置以下设置到默认值：音量(160)、输入源(CD)、平衡(0)、最大音量(255 PGA全量程)、开机上限(192 PGA 0dB)、息屏超时(5分钟)、亮度(80%)、主题(翠绿)、自动输入切换(开)、各输入独立音量(160)、各输入独立平衡(0)、按键映射(恒等: 1-9→1-9)。
...
20. **输入切换音量记忆约定**: 修改输入切换逻辑时，必须区分 `active_input`（实际接通通道）和 `current_input`（目标通道）。保存旧音量只能使用 `active_input`，否则会把旧通道音量写入目标通道的 `input_vol_*`。

21. **页面调节状态清理**: settings_page 和 remote_keys_page 离开页面时需要清理 `settings_active_idx` / `keymap_active_idx` 以及对应行背景/bar 高度。新增返回路径、自动回页路径或长按路径时必须同步处理。

22. **ESPHome lambda guard 约定**: YAML action 列表中，`lambda` 内的 `return` 只退出当前 lambda，不会阻止后续 `script.execute` / `switch.*` action 执行。带 guard 的服务、number、select、BLE 命令入口，必须把后续副作用放进同一个 lambda 的 guard 之后，或用外层 `if:` 包裹。

23. **温度保护唤醒约定**: 所有唤醒路径应通过 `standby_switch->turn_on()` 或显式检查 `!temp_protect`。保护期间不得直接执行 `exit_standby`，解除保护时只根据 `temp_protect_forced_standby` 决定是否自动恢复。

24. **自动输入切换延迟闩锁**: `auto_switch_input` 使用 `auto_switch_pending` 标记真正进入 8 秒等待期。新增音频检测触发路径时，不要绕过该脚本直接延迟切换，避免上一次无效启动留下的延迟段误切输入。
